		...
		'<table class="site_logs">',
			'<tr>',
				...
				'<td class="button" rowspan="2">',
					...
					'<br/>',
					'<input type="submit" value="', i18n::translate('Export'), '" onclick="document.logs.action.value=\'export\';return true;" ', ($action=='show' ? '' : 'disabled="disabled"'),' />',
					'<br/>',
					'<input type="submit" value="', i18n::translate('Delete'), '" onclick="if (confirm(\'', htmlspecialchars(i18n::translate('Permanently delete these records?')) , '\')) {document.logs.action.value=\'delete\';return true;} else {return false;}" ', ($action=='show' ? '' : 'disabled="disabled"'),' />',
				'</td>',
			'</tr>',
		'</table>',
	'</form>';
