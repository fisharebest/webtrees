...
if ($controller->note && $controller->note->canDisplayName()) {
	...
	if ($controller->note->isMarkedDeleted()) {
		if (WT_USER_CAN_ACCEPT) {
			echo '<p class="ui-state-highlight">', WT_I18N::translate('This record has been deleted.  You should review the deletion and then <a href="%1$s">accept</a> or <a href="%2$s">reject</a> it.', $controller->note->getHtmlUrl().'&amp;action=accept', $controller->note->getHtmlUrl().'&amp;action=undo'), '</p>';
		...
		if (WT_USER_CAN_ACCEPT) {
			echo '<p class="ui-state-highlight">', WT_I18N::translate('This record has been edited.  You should review the changes and then <a href="%1$s">accept</a> or <a href="%2$s">reject</a> them.', $controller->note->getHtmlUrl().'&amp;action=accept', $controller->note->getHtmlUrl().'&amp;action=undo'), '</p>';
		} elseif (WT_USER_CAN_EDIT) {
