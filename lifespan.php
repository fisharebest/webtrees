...
<h2><?php echo I18N::translate('Lifespans');?></h2>
	...
	<form class="descriptionbox lifespan-form noprint" name='people' method='post' action='?'>
		<fieldset>
			<legend><?php echo I18N::translate('Select dates or place') . '&hellip;';?></legend>
			<div>
		...
		<fieldset>
			<legend><?php echo '&hellip;' . I18N::translate('or add an individual');?></legend>
			<div>
		...
		<fieldset>
			...
			<div>
				...
				<span class='optionbox total'>
					<?php echo I18N::number($controller->count); ?>
				</span>
			</div>
		</fieldset>
	</form>
...
$controller
	->addInlineJavascript("
		...
		jQuery('#lifespan-chart')
			.width(scale.width())
			.height(Math.ceil(scale.height() + barHeight + $maxY));
		// Prettify the form
		jQuery('fieldset').each(function(i, e){
			maxHeight = Math.max(maxHeight, jQuery(e).height());
		});
		jQuery('fieldset').height(Math.ceil(maxHeight));
	");
