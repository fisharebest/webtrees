...
class AhnentafelReportModuleTest extends TestCase
{
    ...
    public function testReportRunsWithoutError(
        ...
        (new ParserGenerate($xml, $renderer, $vars, $tree, Webtrees::NAME, Registry::timestampFactory()->now()))->process();
    ...
    public function testHtmlSourcesSectionReservesSpaceForFootnotes(): void
    {
        ...
        self::assertStringContainsString('<span class="subheader">Sources</span>', $html);

        // Footnote references appear inline (superscript links)
        $footnote_ref_count = preg_match_all('~<sup>~', $html);
        self::assertGreaterThan(0, $footnote_ref_count);

        // Footnote bodies appear with proper IDs
        $footnote_body_count = preg_match_all('~id="footnote\d+"~', $html);
        self::assertGreaterThan(0, $footnote_body_count);
    }
}
