...
#[CoversClass(Divehi::class)]
class DivehiTest extends AbstractLanguageTestCase
{
    ...
    {
        self::assertSame(Script::Thaa, self::language()->script());
    }

    public function testFirstDay(): void
    {
        self::assertSame(Weekday::Friday, self::language()->firstDay());
    }
    ...
    {
        self::assertSame('dv', self::language()->languageTag());
    }
    public function testEndonym(): void
    {
        self::assertSame('ތާނަ', self::language()->endonym());
    }
    ...
    public function testFormatList(): void
    {
        ...
        self::assertSame('one އަދި two', $language->formatListAnd(['one', 'two']));
        self::assertSame('one، two އަދި three', $language->formatListAnd(['one', 'two', 'three']));

        self::assertSame('', $language->formatListOr([]));
        self::assertSame('one', $language->formatListOr(['one']));
        self::assertSame('one ނުވަތަ two', $language->formatListOr(['one', 'two']));
        self::assertSame('one، two ނުވަތަ three', $language->formatListOr(['one', 'two', 'three']));
    }
    ...
    public function testRelationships(): void
    {
        ...
        // Partners
        self::assertRelationshipNames('އަނބި', 'ފިރިމީހާ', [$husband, $fm, $wife]);
        self::assertRelationshipNames('ކުރީގެ ބައިވެރިޔާ', 'ކުރީގެ ބައިވެރިޔާ', [$wife, $fd, $exHusband]);
        self::assertRelationshipNames('ކައިވެނި ހަމަޖެހިފައި', 'ކައިވެނި ހަމަޖެހިފައި', [$engaged, $fe, $fiance]);

        // Parents
        self::assertRelationshipNames('މަންމަ', 'ފިރިހެން ދަރި', [$son, $fm, $wife]);
        self::assertRelationshipNames('ބައްޕަ', 'ފިރިހެން ދަރި', [$son, $fm, $husband]);
        self::assertRelationshipNames('މަންމަ', 'އަންހެން ދަރި', [$daughter, $fm, $wife]);

        // Siblings
        self::assertRelationshipNames('ކޮއްކޮ', 'ބޭބެ', [$son, $fm, $daughter]);
        self::assertRelationshipNames('ބޭބެ', 'ކޮއްކޮ', [$daughter, $fm, $son]);

        // Half-siblings
        self::assertRelationshipName('އެއްބައްޕަ ދައްތަ', [$son, $fm, $wife, $fd, $stepDaughter]);

        // Stepfamily
        self::assertRelationshipName('ދޮންބައްޕަ', [$stepDaughter, $fd, $wife, $fm, $husband]);
        self::assertRelationshipName('ދޮން އަންހެން ދަރި', [$husband, $fm, $wife, $fd, $stepDaughter]);

        // In-laws
        self::assertRelationshipNames('މައިދައިތަ', 'ޅީ ފިރިހެން ދަރި', [$husband, $fm, $wife, $fw, $motherOfW]);
        self::assertRelationshipNames('ބަފައިކަލުންގެ', 'ޅީ ފިރިހެން ދަރި', [$husband, $fm, $wife, $fw, $fatherOfW]);
        self::assertRelationshipName('ޅީދަރި', [$fatherOfH, $fp, $husband, $fm, $wife]);

        // Grandparents
        self::assertRelationshipNames('މާމަ', 'ކާފަ ދަރި', [$son, $fm, $wife, $fw, $motherOfW]);
        self::assertRelationshipNames('ކާފަ', 'ކާފަ ދަރި', [$son, $fm, $wife, $fw, $fatherOfW]);
        self::assertRelationshipNames('މާމަ', 'ކާފަ ދަރި', [$son, $fm, $husband, $fp, $motherOfH]);
        self::assertRelationshipNames('ކާފަ', 'ކާފަ ދަރި', [$son, $fm, $husband, $fp, $fatherOfH]);

        // Great-grandparents (dynamic)
        self::assertRelationshipName('މުނިކާފަ', [$son, $fm, $husband, $fp, $fatherOfH, $fgp, $paternalGF]);
        self::assertRelationshipName('މުނިމާމަ', [$son, $fm, $husband, $fp, $fatherOfH, $fgp, $paternalGM]);

        // Aunts and uncles — paternal
        self::assertRelationshipNames('ބޮޑުދައިތަ', 'އެއްބަނޑު މީހެއްގެ ފިރިހެން ދަރި', [$son, $fm, $husband, $fp, $sisterOfH]);
        self::assertRelationshipNames('ބޮޑުބައްޕަ', 'އެއްބަނޑު މީހެއްގެ ފިރިހެން ދަރި', [$son, $fm, $husband, $fp, $brotherOfH]);

        // Nieces and nephews
        self::assertRelationshipName('އެއްބަނޑު މީހެއްގެ އަންހެން ދަރި', [$husband, $fp, $brotherOfH, $fbro, $nieceFromBro]);
        self::assertRelationshipName('އެއްބަނޑު މީހެއްގެ ފިރިހެން ދަރި', [$husband, $fp, $brotherOfH, $fbro, $nephewFromBro]);

        // Cousins
        self::assertRelationshipName('ދެބެއިންގެ ދެ ދަރި', [$son, $fm, $husband, $fp, $brotherOfH, $fbro, $cousinFemale]);
        self::assertRelationshipName('ދެބެއިންގެ ދެ ދަރި', [$son, $fm, $husband, $fp, $brotherOfH, $fbro, $cousinMale]);
    }
