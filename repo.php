...
if ($controller->repository && $controller->repository->canDisplayName()) {
	...
	if ($controller->repository->isMarkedDeleted()) {
		if (WT_USER_CAN_ACCEPT) {
			echo '<p class="ui-state-highlight">', WT_I18N::translate('This repository has been deleted.'), WT_I18N::translate('You should review the deletion and then <a href="%1$s">accept</a> or <a href="%2$s">reject</a> it.', $controller->repository->getHtmlUrl().'&amp;action=accept', $controller->repository->getHtmlUrl().'&amp;action=undo'), '</p>';
		} elseif (WT_USER_CAN_EDIT) {
			echo '<p class="ui-state-highlight">', WT_I18N::translate('This repository has been deleted.'), WT_I18N::translate('The deletion will need to be reviewed by a moderator.'), '</p>';
		}
	} elseif (find_updated_record($controller->repository->getXref(), WT_GED_ID)!==null) {
		if (WT_USER_CAN_ACCEPT) {
			echo '<p class="ui-state-highlight">', WT_I18N::translate('This repository has been edited.'), WT_I18N::translate('You should review the changes and then <a href="%1$s">accept</a> or <a href="%2$s">reject</a> them.', $controller->repository->getHtmlUrl().'&amp;action=accept', $controller->repository->getHtmlUrl().'&amp;action=undo'), '</p>';
		} elseif (WT_USER_CAN_EDIT) {
			echo '<p class="ui-state-highlight">', WT_I18N::translate('This repository has been edited.'), WT_I18N::translate('The changes need to be reviewed by a moderator.'), '</p>';
		}
