...
if (empty($to)) {
	echo "<span class=\"error\">".i18n::translate('No recipient user was provided.  Cannot continue.')."</span><br />";
	print_simple_footer();
	exit;
}
		...
		if ($mt>0) {
			...
			if (function_exists("checkdnsrr")) {
				...
				if ($ip === false) {
					...
					$ip = checkdnsrr($host);
					if ($ip === false) {
						echo "<center><br /><span class=\"error\">".i18n::translate('Please enter a valid email address.')."</span>";
						echo "<br /><br /></center>";
						$action="compose";
						//print_simple_footer();
						//exit;
...
if ($action=="compose") {
	echo '<span class="subheaders">'.i18n::translate('Send Message').'</span>';
	$_SESSION["good_to_send"] = true;
	?>
	<script language="JavaScript" type="text/javascript">
		function validateEmail(email) {
			if (email.value.search("(.*)@(.*)")==-1) {
				alert('<?php echo i18n::translate('Please enter a valid email address.'); ?>');
				email.focus();
				return false;
			}
		...
		function checkForm(frm) {
			if (frm.subject.value=="") {
				alert('<?php echo i18n::translate('Please enter a message subject.'); ?>');
				document.messageform.subject.focus();
				return false;
			}
			if (frm.body.value=="") {
				alert('<?php echo i18n::translate('Please enter some message text before sending.'); ?>');
	...
	echo "<tr><td valign=\"top\" align=\"right\">".i18n::translate('Body:')."<br /></td><td><textarea name=\"body\" cols=\"50\" rows=\"7\">$body</textarea><br /></td></tr>";
	echo "<tr><td></td><td><input type=\"submit\" value=\"".i18n::translate('Send')."\" /></td></tr>";
	echo "</table>";
	echo "</form>";
	if ($method=="messaging2") echo i18n::translate('When you send this message you will receive a copy sent via email to the address you provided.');
}
else if ($action=="delete") {
	if (deleteMessage($id)) echo i18n::translate('Message Deleted');
}
