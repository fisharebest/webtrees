...
if ($controller->record && $controller->record->canShow()) {
	...
	if ($controller->record->isPendingDeletion()) {
		if (Auth::isModerator($controller->record->getTree())) {
			...
			echo
				'<p class="ui-state-highlight">',
				I18N::translate('This record has been deleted.  The deletion will need to be reviewed by a moderator.'),
				' ', FunctionsPrint::helpLink('pending_changes'),
				'</p>';
		}
		...
		if (Auth::isModerator($controller->record->getTree())) {
			...
			echo
				'<p class="ui-state-highlight">',
				I18N::translate('This record has been edited.  The changes need to be reviewed by a moderator.'),
				' ', FunctionsPrint::helpLink('pending_changes'),
				'</p>';
		}
	}
