...
if ($controller->source && $controller->source->canDisplayName()) {
	...
	if ($controller->source->isMarkedDeleted()) {
		if (WT_USER_CAN_ACCEPT) {
			echo '<p class="ui-state-highlight">', WT_I18N::translate('This record has been deleted.  You can <a href="%1$s">accept</a> or <a href="%2$s">reject</a> this deletion.', $controller->source->getHtmlUrl().'&amp;action=accept', $controller->source->getHtmlUrl().'&amp;action=undo'), '</p>';
		...
		if (WT_USER_CAN_ACCEPT) {
			echo '<p class="ui-state-highlight">', WT_I18N::translate('This record has been changed.  You can <a href="%1$s">accept</a> or <a href="%2$s">reject</a> the changes.', $controller->source->getHtmlUrl().'&amp;action=accept', $controller->source->getHtmlUrl().'&amp;action=undo'), '</p>';
		} elseif (WT_USER_CAN_EDIT) {
