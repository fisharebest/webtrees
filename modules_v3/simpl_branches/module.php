...
class simpl_branches_WT_Module extends WT_Module implements WT_Module_Config, WT_Module_Menu {
		
	// Extend WT_Module
	public function getTitle() {
		return /* I18N: Name of a module  */ WT_I18N::translate('Simpl Branches');
	}
	...
	// Get the stylesheet - the stylesheet has no specific theme styles so we use the same for all themes
	private function getStyleSheet() {		
		$stylesheet ='	if (document.createStyleSheet) {
	...
	public function getMenu() {
		...
		if (WT_SCRIPT_NAME == 'branches.php') {
			$surn = WT_Filter::get('surname', WT_REGEX_ALPHA);
			if ($surn) {
			...
			$controller
				...
				->addInlineJavaScript($this->getStyleSheet().'			
				jQuery("fieldset")
					.before("<h2>'.$controller->getPageTitle().'</h2>")
