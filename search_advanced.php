...
?>
	...
	function showDate(sel, row) {
		...
		opt = document.createElement('option');
		opt.value='';
		opt.text=<?= json_encode(I18N::plural('±%s year', '±%s years', 2, I18N::number(2))) ?>;
		sel.appendChild(opt);
		opt = document.createElement('option');
		opt.value='5';
		opt.text=<?= json_encode(I18N::plural('±%s year', '±%s years', 5, I18N::number(5))) ?>;
		sel.appendChild(opt);
		opt = document.createElement('option');
		opt.value='10';
		opt.text=<?= json_encode(I18N::plural('±%s year', '±%s years', 10, I18N::number(10))) ?>;
		sel.appendChild(opt);
