...
if ($tree1_id && $tree2_id != $tree1_id) {
	...
	if ($xrefs) {
		...
		echo
			...
			'</p>',
			'<p>',
			'<a class="current" href="' . e(Html::url('admin_trees_renumber.php', ['ged' => $tree1->getName()])) .'">',
			I18N::translate('Renumber family tree'), ' — ', e($tree1->getTitle()),
			'</a>',
			'</p>',
			'<p>',
			'<a class="current" href="' . e(Html::url('admin_trees_renumber.php', ['ged' => $tree2->getName()])) .'">',
		...
		Database::exec(
			...
			" `##media_file` WRITE," .
			" `##media_file` AS media_file2 READ," .
			" `##other` WRITE," .
