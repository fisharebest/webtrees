...
class Request extends SymfonyRequest implements ServerRequestInterface
{
    ...
    public function getUploadedFiles(): array
    {
        ...
        foreach ($this->files as $key => $file) {
            if ($file instanceof UploadedFile) {
                $stream = app(StreamFactoryInterface::class)->createStreamFromFile($file->getPathName());

                $files[$key] = app(UploadedFileFactoryInterface::class)->createUploadedFile(
                    $stream,
                    $file->getSize(),
                    $file->getError(),
    ...
    public function withUploadedFiles(array $uploadedFiles)
    {
        ...
        foreach ($uploadedFiles as $file) {
            ...
            $files[] = app(UploadedFileFactoryInterface::class)->createUploadedFile(
                $filename,
