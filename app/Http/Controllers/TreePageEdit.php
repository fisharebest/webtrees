...
final class TreePageEdit
{
    ...
    public function get(Tree $tree): ResponseInterface
    {
        ...
        $all_blocks = $this->home_page_service->availableTreeBlocks($tree, $this->user);
        $title      = I18N::translate('Change the "Home page" blocks');
        $url_cancel = route(TreePage::class, ['tree' => $tree->name()]);
        $url_save   = route(self::class, ['tree' => $tree->name()]);
    ...
    public function post(ServerRequestInterface $request, Tree $tree): ResponseInterface
    {
        ...
        if ($defaults) {
            ...
            $main_blocks = $this->home_page_service->treeBlocks($default_tree, $this->user, ModuleBlockInterface::MAIN_BLOCKS)
                ->map(static fn (ModuleBlockInterface $block) => $block->name());
            $side_blocks = $this->home_page_service->treeBlocks($default_tree, $this->user, ModuleBlockInterface::SIDE_BLOCKS)
                ->map(static fn (ModuleBlockInterface $block) => $block->name());
        } else {
