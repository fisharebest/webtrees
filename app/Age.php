...
class Age
{
    ...
    const KEYWORD_STILLBORN = 'stillborn';

    // GEDCOM keyword: age < 1 year
    const KEYWORD_INFANT = 'infant';

    // GEDCOM keyword: age < 8 years
    const KEYWORD_CHILD = 'child';
    ...
    public function __construct(string $age)
    {
        ...
        // Weeks are a non-standard extension.
        $this->years  = $this->extractNumber($age, 'y');
        $this->months = $this->extractNumber($age, 'm');
        $this->weeks  = $this->extractNumber($age, 'w');
        $this->days   = $this->extractNumber($age, 'd');
    }
