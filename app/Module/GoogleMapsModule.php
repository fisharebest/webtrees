	...
	private function pedigreeMapJavascript() {
		...
		function createMarker(point, name, html, mhtml, icontype) {
			...
			var marker = new google.maps.Marker({
				icon:     new google.maps.MarkerImage(WT_STATIC_URL+WT_MODULES_DIR+"googlemap/images/icon" + icontype + ".png"),
				map:      pm_map,
				position: point,
				id:       index,
				html:	  mhtml,
		...
		for ($i = 0; $i < $this->treesize; $i++) {
			...
			if ($person) {
				...
				$latlongval = $this->getLatitudeAndLongitudeFromPlaceLocation($person->getBirthPlace());
				if ($latlongval) {
					$lat[$i] = (double) strtr($latlongval->pl_lati, array('N' => '', 'S' => '-', ',' => '.'));
					$lon[$i] = (double) strtr($latlongval->pl_long, array('E' => '', 'W' => '-', ',' => '.'));
					if ($lat[$i] && $lon[$i]) {
						$js .= 'var point = new google.maps.LatLng(' . $lat[$i] . ',' . $lon[$i] . ');';
						$js .= 'var marker = createMarker(point, "' . Filter::escapeJs($name) . '","' . $data . '", "';
						$js .= '<div class=\"gm-info-window\">' . $data . '</div>", "' . $curgen . '");';
