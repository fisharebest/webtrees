...
class OpenStreetMapModule extends AbstractModule implements ModuleConfigInterface, ModuleTabInterface, ModuleChartInterface {
	 ...
	 * @return array
	 */
	public static function assets($type = 'user') {
	 ...
	 * @param string $type
	 */
	private static function loadAssets(PageController $controller, $type = 'user') {
			...
			foreach ($places as $id => $place) {
				...
				if ($location->hasChildren()) {
					$name = '<div class="osm-name">
							    <a href="' .
						            e(route('place-hierarchy', ['parent' => $url_parent, 'ged' => $tree->getName()])) . '">'
                                    . $img . ' ' . $place->pl_place .
								'</a>
							  </div>';
				} else {
	...
	private function adminPlaceEdit() {
		...
		$controller = new PageController;
		self::loadAssets($controller, 'admin');
		$controller
