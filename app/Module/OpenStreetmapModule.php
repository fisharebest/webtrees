...
class OpenStreetMapModule extends AbstractModule implements ModuleConfigInterface, ModuleTabInterface, ModuleChartInterface {
	...
	private function pedigreeMap() {
		...
		$controller
			->restrictAccess(Module::isActiveChart($tree, $this->getName()))
			->setPageTitle(/* I18N: %s is an individual’s name */
				I18N::translate('Pedigree map of %s', $controller->root->getFullName())
			)
			->pageHeader()
			->addInlineJavascript(
				'WT_OSM.drawMap("' . $individual->getXref() . '","' . $tree->getTreeId() . '", "pedigree","' . $generations . '");'
			);
		...
		<div id="pedigreemap-page">
			<h2 class="wt-page-title"><?= $pagetitle ?></h2>

			<form class="wt-page-options wt-page-options-pedigree-map d-print-none">
				<input type="hidden" name="ged" value="<?= e($tree->getName()) ?>">
				<input type="hidden" name="mod" value="<?= e($this->getName()) ?>">
			...
			<div class="wt-map-tab py-4">
				<div class="gchart osm-wrapper row">
				...
				<?php if ($is_admin): ?>
					<div class="osm-options">
						...
						e(
							Html::url(
							'module.php',
							['mod' => $this->getName(), 'mod_action' => 'admin_config']
						)
						)
						?>">
						...
						e(
							Html::url(
							'module.php',
							['mod' => $this->getName(), 'mod_action' => 'admin_places']
						)
						)
						?>">
	...
	private function placelistGetMapData() {
		...
		$placeObj  = new Place($rootid, $tree);
		...
		foreach ($places as $id => $place) {
			...
			if ($location->isValid() && $location->knownLatLon()) {
				//Stats
				$placeStats = [];
				...
				if ($location->getIcon() !== null && is_file($flag_path . $location->getIcon())) {
					$flag = '<img src="' . $flag_path . $location->getIcon() . '">';
				} else {
					$flag = '';
				}
				if ($showlink) {
					$name = '<div class="label">
							    <a href="' . $place->getURL() . '">' . $flag . ' ' . $place->getPlaceName()  . '</a>
							  </div>';
				} else {
					$name = '<div class="label">' . $flag . ' ' . $place->getPlaceName() . '</div>';
				}
		...
		$this->ajaxResponse($code, ['type' => 'FeatureCollection', 'features' => $features]);
	}
